...
This repository builds `esmwell` — an ESM-only in-browser code runner with judge, REPL, one-shot module-project, and lazy Vitest/Jest workspace modes — as a pnpm/Turborepo monorepo maintained with Vitest, tsdown, oxfmt, oxlint, Changesets, and mise. The only publishable package lives in `packages/esmwell`; the workspace root and every demo under `apps/` must stay private and excluded from Changesets publishing.

Preserve the runner's invariants: submitted judge and REPL modules run only inside a same-origin child execution worker, never inside their coordinator worker. Each judge run gets a fresh child. A REPL child persists only until reset, timeout, fatal failure, or session close. Named ESM declarations can seed its persistent scope; local export lists add no bindings, while re-exports remain unsupported. Missing REPL identifiers report `ReferenceError`, while a direct `typeof` check returns `'undefined'`. Module projects and test workspaces use one fresh directly owned worker per run because their service-worker-backed module graph must work in Chromium and WebKit. Closing either session terminates all active run workers, settles pending runs as errors, and prevents asynchronous setup from starting a late worker; graph caches are cleaned before run settlement. Runtime-owned global bindings cannot be overwritten, redefined, or deleted, while intended contents such as `process.env` remain mutable. Bare imports resolve via esm.sh (inline versions override `deps`, which override `autoInstall`), `process`, `node:process`, and `globalThis.process` expose the same browser-identified process object, module graphs keep runtime-owned specifiers reserved and resolve other exact canonical local ids before packages, test workspaces load official engine components lazily, and error messages stay self-contained and actionable. `import.meta.main` is `true` in a module project's `entry` (and its same-source `.js`/`.mjs`/`.cjs` alias), in a judge module, and in each test file, and `false` everywhere else including REPL inputs; it is set by rewriting `import.meta` in place so reported line numbers never shift.

Every session accepts a main-thread `transform` that rewrites submitted source before it is posted to the worker, in submission order, with failures surfacing as error results. Module-project transforms receive `{ kind: 'project', id }`. `esmwell/typescript` builds one on `ts.transpileModule` from a compiler the host supplies through `load`, so the package never bundles or depends on TypeScript. `esmwell/utils` exports `canonicalModuleId` and `createProjectModules` for editor paths, validated ids, script aliases, and collision errors without compiling sources. It also exports the worker RPC pair (`createWorkerRpc`, `serveWorkerRpc`) and the console formatter so hosts stop re-implementing them.

The private playground uses CodeMirror 6 for both source and REPL input. Its `.ts` mode compiles in a dedicated language-service worker, reached through esmwell's worker RPC and wired into each session as its `transform`; that worker uses the opt-in `esmwell/typescript-editor` kit to acquire and cache exact-version npm declaration graphs on demand for hover and completion, and seeds the kit's `ESMWELL_RUNTIME_TYPES` as a root script file because `import.meta.main` comes from the runner rather than from a package. That declaration matches `@types/node`'s shape, so a host loading both keeps one declaration of the flag. The kit exports `createTypeScriptTypeGraphAdapter` to replace acquired virtual files and resolve package-scoped declarations; hosts own their language service and invalidate project and declaration versions when its `apply` returns true. The kit stays editor-neutral and accepts a structural `preProcessFile` compiler capability, so TypeScript remains host-supplied. `.mjs` shows the generated JavaScript and passes it directly. TypeScript edits invalidate the generated view, invalid TypeScript cannot open `.mjs`, and direct JavaScript edits lock `.ts` until the source restore control resets both views. The CodeMirror language-service UI remains a private playground authoring feature.
...
pnpm -C packages/esmwell run check:size    # 30 KB gzip budget over built runner-core ESM output; run after pnpm build
