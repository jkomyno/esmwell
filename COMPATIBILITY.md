...
| TypeScript, TSX, and JSX source     |   ❌   | Submitted modules must already be JavaScript. runesm does not ship a compiler or syntax transform.                                                                                                                                                     |
...
| REPL missing names                   |   ⚠️   | A name never declared in the REPL resolves to `undefined` through the scope object instead of throwing `ReferenceError`.                                |
