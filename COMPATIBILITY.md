...
| ---------------------------- | :----: | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ESM packages from esm.sh     |   ✅   | Zod 4 and Effect 4 beta resolve and execute in browser workers. The suite imports all 156 `effect@beta` entry points, then runs Schema decoding, `Effect.runFork`, scoped `Effect.acquireRelease`, `ConfigProvider.fromEnv`, `Schema.File`, MessagePack, `Path.layer`, and `FetchHttpClient`. |
| Runtime code generation      |   ✅   | Zod 4 builds validators with `new Function` inside the package: the suite runs both `z.compile(schema, { strict: true })` and the `zod/compile` global mode. The policy below rejects that construct in submitted source only, and a host CSP without `unsafe-eval` blocks it.                |
...
| Direct `Function` constructor calls |   ❌   | Direct calls, construction, tags, and direct `.call()`, `.apply()`, or `.bind()` constructor invocations are rejected. This policy is a code-quality gate, not a security claim. Dependency code is never scanned, so a package such as Zod 4 may still generate validators at runtime. |
...
| `import.meta.main`                  |   ✅   | `true` in a module project's `entry` (and its same-source `.js`/`.mjs` twin), in a judge module, and in each test file of a workspace; `false` in every other module and in REPL inputs. `import.meta.url` and `import.meta.resolve` keep their native values.                          |
...
### Why are `eval`, the `Function` constructor, and `var` rejected?
