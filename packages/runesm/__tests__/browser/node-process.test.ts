import { createReplSession, createRunesm } from '/runesm/index.mjs'
...
test('implements the documented partial node:process semantics', async () => {
  const result = await runProcessProbe(
    `
      ...
      export const solve = async () => {
        ...
        const listenerMethodsReturnProcess = [
          process.on('runesm', listener),
          process.once('runesm', listener),
          process.off('runesm', listener),
          process.addListener('runesm', listener),
          process.removeListener('runesm', listener),
        ].every((value) => value === process)
        const emitted = process.emit('runesm', 1)

        process.env.RUNESM_MODE = 'browser'
        process.env.RUNESM_NUMBER = 42
    ...
    `,
    {
      ...
      cwd: '/',
      emitted: false,
      env: { RUNESM_MODE: 'browser', RUNESM_NUMBER: 42 },
...
test('starts every judge run with fresh process env and argv contents', async () => {
  ...
  try {
    const mutated = await session.runJudge(
      `export const solve = () => {
        process.env.RUNESM_MODE = 'first'
        process.argv.push('first.js')
        return { env: process.env.RUNESM_MODE, argv: process.argv }
      }`,
...
test('persists REPL process contents until reset replaces the child realm', async () => {
  ...
  try {
    const mutated = await session.evaluate(
      `process.env.RUNESM_MODE = 'repl'
       process.argv.push('repl.js')`,
    )
    assert(mutated.ok === true, `REPL should mutate process contents: ${mutated.error?.message}`)

    const persistent = await session.evaluate(`({ env: process.env.RUNESM_MODE, argv: process.argv })`)
