 ...
 * untouched.
 */
const SCOPE_MODULE_SOURCE = `const __esmwellScope = Object.create(null)
const __esmwellBoundGlobals = new WeakMap()
export const __esmwell = new Proxy(__esmwellScope, {
  get(target, key) {
    ...
    let bound = __esmwellBoundGlobals.get(value)
    if (bound === undefined) {
      ...
      __esmwellBoundGlobals.set(value, bound)
    }
    return bound
  },
...
export const __esmwellTypeof = new Proxy(__esmwellScope, {
  get(target, key) {
    if (key in target) return target[key]
