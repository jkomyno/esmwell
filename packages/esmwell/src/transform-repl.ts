 ...
 *
 * ```js
 * import { __esmwell, __esmwellTypeof } from '<scopeModuleUrl>'
...
function rewriteImportStatement(
  ...
  const moduleVar = `__esmwell_mod_${index}`
    ...
    for (const declarator of readNodeChildren(statement, 'declarations')) {
      ...
      if (id.type === 'Identifier') {
        ...
        if (id.type === 'ObjectPattern') {
          // `{ __esmwell.a } = src` at statement position parses as a block;
    ...
    case 'ObjectPattern':
      for (const property of readNodeChildren(pattern, 'properties')) {
        ...
        if (shorthand && key !== null) {
          // `{ a }` → `{ a: __esmwell.a }`; `{ a = 1 }` → `{ a: __esmwell.a = 1 }`.
