import { createTestSession } from '/esmwell/index.mjs'
  ...
  'tests/impl.test': `
    ...
    describe('Zod 4 workspace', () => {
      it('imports and validates a local implementation', () => {
        expect(parseName({ name: 'esmwell' })).toBe('ESMWELL')
      })
      ...
      it('protects the esmwell test API binding', () => {
        const descriptor = Object.getOwnPropertyDescriptor(globalThis, '__esmwellTestApi')
        expect(descriptor?.configurable).toBe(false)
        expect(descriptor?.writable).toBe(false)
        expect(Reflect.set(globalThis, '__esmwellTestApi', null)).toBe(false)
        expect(Reflect.deleteProperty(globalThis, '__esmwellTestApi')).toBe(false)
        expect(() => Object.defineProperty(globalThis, '__esmwellTestApi', { value: null })).toThrow()
        ${
          engine === 'vitest'
...
test('runs current Vitest and Jest engines over local ESM modules with Zod 4', async () => {
  for (const engine of ['vitest', 'jest'] as const) {
    const session = createTestSession({
      workerUrl: '/esmwell/test-worker-entry.mjs',
