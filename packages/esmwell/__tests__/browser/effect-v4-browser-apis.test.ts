  ...
  const result = await runEffect(
    `
      ...
      export const solve = () => {
        ...
        return Effect.runSync(Config.string('ESMWELL_MODE').parse(provider))
      }
    `,
    'browser',
  )
...
test('round-trips MessagePack through Effect unstable encoding', async () => {
  const result = await runEffect(
    `
      ...
      export const solve = () => {
        const encoded = Schema.encodeSync(Codec)({ id: 7, label: 'esmwell' })
    ...
    `
      ...
      export const solve = () => Effect.runPromise(
        Effect.flatMap(HttpClient.get(location.origin + '/'), (response) => response.text).pipe(
          Effect.map((body) => body.includes('<title>esmwell browser tests</title>')),
