...
Add a `transform` option to every session and a `runesm/typescript` entry.
...
`typescriptTransform` from `runesm/typescript` compiles TypeScript with `ts.transpileModule` using a compiler the host supplies through `load: () => import('typescript')`, so the package never bundles or depends on the compiler.
